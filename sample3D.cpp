...
void motor()
{
	...
	py=(y+ly)/scale;
	px+=(_terrain->width())/2;
	py+=(_terrain->length())/2;
	glTranslatef(x+lx,y+ly, _terrain->getHeight(px,py)*scale+1.6);
	glutSolidSphere(0.25, 20, 20);
	glPushMatrix();
	GLUquadricObj *quadratic;
	glColor3f(1.0, 0.0, 0.0);
	quadratic = gluNewQuadric();
	glRotatef((angle)*(180/3.14),0.0,0.0,1.0);
	glTranslatef(-0.5,0,0);
	glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
	gluCylinder(quadratic,0.1f,0.1f,1.0f,22,22);
	glPopMatrix();
	glPushMatrix();
	...
	gluCylinder(quadratic1,0.1f,0.1f,1.0f,22,22);
	glPopMatrix();
	...
	glRotatef(90.0f, 0.0f, 1.0f, 0.0f);
	glRotatef(-angle*(180/3.14), 1.0f, 0.0f, 0.0f);
	glutSolidTorus(2.0/20,8.5/20,5,100);
	glPopMatrix();
}
//----------------------------------------------------------------------
...
//	cout<<scale<<endl;
	...
	if(v==0)
	{
		...
		py+=(_terrain->length())/2;
		gluLookAt(
				x,      y,       _terrain->getHeight(px,py)*scale+2.0,
				x + lx, y + ly,  _terrain->getHeight(px,py)*scale+2.0,
				0.0,    0.0,    1.0);
	}
	if(v==1)
	{
		...
		py+=(_terrain->length())/2;
		gluLookAt(
				x+1.3*lx,y+1.3*ly, _terrain->getHeight(px,py)*scale+1.3,
				3*lx+x,3*ly+y, _terrain->getHeight(px,py)*scale+1.3,
				0.0,    0.0,    1.0);
	}
	if(v==2)
	{
		...
		py+=(_terrain->length())/2;
		gluLookAt(
				x-5*lx,y-5*ly, _terrain->getHeight(px,py)*scale+3,
				x+lx, y+ly,  _terrain->getHeight(px,py)*scale+3,
				0.0,    0.0,    1.0);
	}
	if(v==3)
	{
		...
		py+=(_terrain->length())/2;
		gluLookAt(
				x,y, _terrain->getHeight(px,py)*scale+10,
				x, y,  _terrain->getHeight(px,py)*scale+2,
				lx,    ly,    0.0);
	...
	for(int i = -3; i < 3; i++)
		for(int j = -3; j < 3; j++) {
			glPushMatrix();
//			glTranslatef(i*7.5, j*7.5, _terrain->getHeight(_terrain->width()-0.1,_terrain->length()-0.1));
			float px,py;
			px=i*7.5/scale;
			py=j*7.5/scale;
			px+=(_terrain->width())/2;
			py+=(_terrain->length())/2;
			glTranslatef(i*7.5, j*7.5, _terrain->getHeight(px,py)*scale);
...
//	glTranslatef(0.0f, -_terrain->width()/2,0.0 );
	glTranslatef(-(float)(_terrain->width() - 1) / 2,-(float)(_terrain->length() - 1) / 2,0.0);
	...
	glutSolidSphere(5, 10, 10); // right eye
	glTranslatef(_terrain->width(),_terrain->length(),_terrain->getHeight(_terrain->width()-0.1,_terrain->length()-0.1));
	glutSolidSphere(5, 10, 10); // right eye
	glutSwapBuffers();
...
void processNormalKeys(unsigned char key, int xx, int yy)
{
	...
	{   
		v++;
		v%=4;
		//cout<<_terrain->getHeight(-_terrain->width(),-_terrain->length());
	}  
} 
